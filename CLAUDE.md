...
> **Status, 31 Aug — this section is largely historical.** `docs/QOL-BACKLOG.md` is the live
> backlog and it is nearly all done. Of the roadmap items below: the **P0 detection rework** is
> still the biggest open piece (built as opt-in Diagnostics toggles, never confirmed live);
> **P1 bard-song handling** all shipped; **P2 quick wins** all shipped; the **custom-timer form
> overhaul** shipped (Add-Aura modal rework still open); the **Bard Songs aura** is now a hybrid
> buff/debuff feed. **Action Bars is no longer just a stub** — gem drag-to-swap, configured-slot
> markers, and the move HUD for action bars all shipped; only the *cover-image* replacement layer
> (custom art / border frames over hotbar buttons) is unbuilt. Multi-step aura type and the
> unified aura-scale control are still unbuilt.
