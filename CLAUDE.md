...
- `src/main/raidNamedTracker.js` — the board's engine: own `currentZone`, a `changed` event, rebuilds the board on zone change; a slain line greys a named; `setZone()` is also called by the startup zone-recovery. A new `raidNamed` buff source + `raid-named-builtin` aura kind + its own `SHAPE_FIELDS` entry; `overlay.js` dims killed rows. **Since c3479d4 the board shows on ANY entry to a tracked zone** (a plain `You have entered X.`), raid or group — the `raid: true` flag and `viaVoidling` are metadata only, they never gate visibility (line 91 below is stale). **Survives a restart (4 Sep):** `captureState` / `restoreState` + a `sessionRestore` `raidNamed` registration (30-min grace) — without it every mid-raid restart rebuilt the board all-nameds-up and lost the group's kill progress (confirmed from the Sept 4 debug log: 3 restarts in one Plane of Hate raid, each forgetting 7-8 kills). Startup order: `restoreAll` runs before the log-tail zone recovery, so `restoreState` stashes the snapshot and `_enterZone` applies it once the matching zone's board exists (`_seeding` guards a live Voidling re-entry from re-greying — that's a genuinely fresh instance). An entry with a difficulty suffix but no named list now logs a loud diagnostic instead of a silent empty board.
...
53. **Inside a loadout-locked zone a gem memorise is TRUTH, not the usual weak `currentlyMemorized` guess.** Owner, 4 Sep: a full EQL loadout swap is impossible in a dungeon / raid-group instance, so a `"You have finished memorizing X."` seen there means the gem really holds that spell — a mob singing a same-named ability, or a shared landing text, can't make it hers-or-not. `src/shared/loadoutLockedZones.js` `isLoadoutLockedZone(rawZone)` = any private-instance difficulty suffix (`- Group`, `N (Awakened)`) **or** a `RAID_ZONE_NAMEDS` key (the baseline dungeon/raid list; `LOADOUT_LOCKED_EXTRA` for the rest — "there are others"). `main.js` calls `buffEngine.setLoadoutLocked(locked, baseZoneName)` on every zone change + startup. `buffEngine._gemVerified` holds names memorised **this visit only** — wiped on any real zone change (base-name keyed, so an entrance→instance / reconnect echo doesn't clear it) and on a profile switch, because the owner's case is stepping out, swapping loadout with no log line, and stepping back in: re-entry must **not** reinstate the old gems. Two uses: a verified gem resolves an ambiguous self-landing the spellbook can't (new tier, runs before the spellbook check), and `_attributeBardSongCaster` returns `'You'` for a verified-gem song ahead of `_recentOtherCaster` (kills gotcha #31's stale-mob-cast path for the locked-zone weave). Startup seeds only the *flag*, never the gems (a memorise before app-start was never observed). The replay harness never calls `setLoadoutLocked`, so the feature is inert there — unit-tested in `test/loadout-locked-zones.test.js`.
