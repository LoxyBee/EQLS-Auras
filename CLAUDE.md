...
- `src/shared/data/zoneGraph.js` — 104 zones, their connections (`land`/`boat`/`portal`) and 61 travel spells, for note 20. Sourced EQL-specific and cross-checked against three others; **38 display names are inferred and flagged `nameConfidence: 'inferred'`** because the player has never entered those zones. Generated once from research; nothing regenerates it, so edits belong in the file.
