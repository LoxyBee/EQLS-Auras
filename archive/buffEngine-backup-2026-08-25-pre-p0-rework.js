// BACKUP - 25 Aug 2026, taken right after the P0 rework's first piece shipped (the
// useEvidenceModel toggle on the unique-landing-text tier's soft-negative branch) and right
// before any further P0 work (P0b caster attribution, or any other change to handleLine()).
//
// This is a straight copy, not a require()'d module - nothing in src/ imports this file, and nothing
// should. It exists purely so a live regression in src/main/buffEngine.js can be undone in one step:
// stop the app, copy this file's contents over src/main/buffEngine.js, restart. Kept outside src/ so
// it never ships, same reasoning as archive/buffs-legacy-11337.json.
//
// Do not edit this file. If you need to compare it against the live version, use git diff instead -
// this copy is a snapshot, not a maintained fork.
  ...
  handleLine(line) {
    ...
    if (uniqueMatch) {
      ...
      if (gate && neverScribed) {
        if (this.trackOthersEnabled) {
          this._debugLog(`LANDED "${uniqueMatch.name}" - unique text, but never scribed by you at all; landed anyway (track others ON)`);
          this._land(uniqueMatch);
        } else {
      ...
      // still treats them as a hard veto, unchanged from before. With the model on, they can only
      // demote a silent LAND into a queued prompt - never into a silent IGNORE - so a real buff
      // that used to vanish with no trace now at least surfaces for you to confirm.
      if (gate && (alliesBursting || staleGem)) {
        if (this.useEvidenceModel) {
          const reason = alliesBursting ? "an ally's instant grant just fired" : 'not currently memorized by you';
          ...
          if (rememberedBuff) {
            this._debugLog(`LANDED "${rememberedBuff.name}" - remembered choice for "${stripped}" (your cast, soft evidence otherwise: ${reason})`);
            this._land(rememberedBuff);
          } else {
            this._debugLog(`QUEUED "${uniqueMatch.name}" - unique text but ${reason} (soft evidence, evidence-based detection ON)`);
            this._queueAmbiguousCast(stripped, [uniqueMatch], true);
          }
        } else if (this.trackOthersEnabled) {
          this._debugLog(
            alliesBursting
              ? `LANDED "${uniqueMatch.name}" - unique text, but an ally's instant grant just fired; landed anyway (track others ON)`
              : `LANDED "${uniqueMatch.name}" - unique text, but not currently memorized by you; landed anyway (track others ON)`
          );
          this._land(uniqueMatch);
        } else {
          this._debugLog(
            alliesBursting
              ? `IGNORED "${uniqueMatch.name}" - unique text, an ally's instant grant just fired, track others OFF`
              : `IGNORED "${uniqueMatch.name}" - unique text, not currently memorized by you, track others OFF`
          );
        }
        this._checkForEndedBuffs(line);
        return;
