 ...
 *   1. a backup is written BEFORE anything is replaced
 *   2. buffs the user made themselves survive
 *   3. per-buff "show on overlay" choices survive for spells that exist in both rosters
 *
...
test('a backup is written before the roster is replaced', () => {
  const store = legacyStore();
  const before = JSON.parse(JSON.stringify(store.data.buffs));
  new BuffStore(store);
  const backup = store.data['buffs-backup-before-eql-roster'];
  assert.ok(backup, 'no backup was written - a bad migration would be unrecoverable');
  assert.deepEqual(backup, before, 'the backup does not match what was there beforehand');
});
...
test('it runs exactly once', () => {
  ...
  const backupAfterFirst = JSON.parse(JSON.stringify(store.data['buffs-backup-before-eql-roster']));
  ...
  assert.deepEqual(
    store.data['buffs-backup-before-eql-roster'], backupAfterFirst,
    'the backup was overwritten on a later launch, destroying the only copy of the original roster'
  );
