...
test('RaidNamedTracker.setZone rebuilds the board for a raid zone, "- Group" or not (owner, 2 Sep / 13 Sep)', () => {
  const t = new RaidNamedTracker();
  try {
    // The Plane of Fear is a `raid: true` entry, but "anything that is a RAID is also a separate
    // DUNGEON" - the board shows on a plain group entry too. The zone string's own "- Group"
    // marker is what sets `viaVoidling` - not a hail, which setZone never sees at startup anyway.
    t.setZone('The Plane of Fear 4 (Refined)');
    ...
    t.setZone('The Plane of Fear - Group 4 (Refined)'); // the raid-lockout instance -> reset + flagged
...
test('RaidNamedTracker.setZone rebuilds a DUNGEON board with no confirmation needed', () => {
  const t = new RaidNamedTracker();
  try {
    t.setZone("Nagafen's Lair 4 (Refined)"); // group dungeon - no `raid` flag
